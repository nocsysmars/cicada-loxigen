...
extension_message_subtype = {
    ...
    of_g.VERSION_1_3:dict(  # Version 1.0 extensions
        ...
        ofdpa = {   # ofdpa extensions; indexed by class name, value is subtype
            },
        ),
}
...
extension_action_subtype = {
    ...
    of_g.VERSION_1_3:dict(  # Version 1.0 extensions
        ...
        # OFDPA_FIXUP
        ofdpa = {   # of_action_ofdpa_
            },
        onf = {   # of_action_ofdpa_
            },
        nicira = {   # of_action_nicira_
            }
        ),
...
# OFDPA_FIXUP
# Set to empty dict if no extension oxm defined
extension_oxm_subtype = {
    # version 1.0
    of_g.VERSION_1_0:dict(),
    of_g.VERSION_1_1:dict(),
    of_g.VERSION_1_2:dict(),
...
# OFDPA_FIXUP
# Set to empty dict if no extension meter_band defined
extension_meter_band_subtype = {
    # version 1.0
    of_g.VERSION_1_0:dict(),
    of_g.VERSION_1_1:dict(),
    of_g.VERSION_1_2:dict(),
    of_g.VERSION_1_3:dict(
        ofdpa = {   # of_meter_band_ofdpa_
            }
        ),
}
...
extension_objects = [
    ...
    extension_instruction_subtype,
    extension_instruction_id_subtype,
    extension_oxm_subtype, # OFDPA_FIXUP
    extension_meter_band_subtype, # OFDPA_FIXUP
    extension_queue_prop_subtype,
    extension_table_feature_prop_subtype
]
...
# OFDPA_FIXUP start
################################################################
# These are extension oxm specific
################################################################

def oxm_is_extension(cls, version):
    """
    Return True if cls, version is recognized as an oxm extension
    This is brute force, searching records for a match
    """
    return cls_is_ext_obj(cls, version, extension_oxm_subtype)

def extension_oxm_to_subtype(cls, version):
    """
    Return the ID of the oxm subtype (for its experimenteer)
    if class is an oxm extension, else None
    """
    if version in extension_oxm_subtype:
        for exp, classes in extension_oxm_subtype[version].items():
            if cls in classes:
                return classes[cls]
...
################################################################
# These are extension meter_band specific
################################################################

def meter_band_is_extension(cls, version):
    """
    Return True if cls, version is recognized as an meter_band extension
    This is brute force, searching records for a match
    """
    return cls_is_ext_obj(cls, version, extension_meter_band_subtype)

def extension_meter_band_to_subtype(cls, version):
    """
    Return the ID of the meter_band subtype (for its experimenteer)
    if class is an meter_band extension, else None
    """
    if version in extension_meter_band_subtype:
        for exp, classes in extension_meter_band_subtype[version].items():
            if cls in classes:
                return classes[cls]
